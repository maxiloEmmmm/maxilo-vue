...
(function webpackUniversalModuleDefinition(root, factory) {
   ...
   */
       ...
       * Mock.mock( rurl, rtype, function(options) )
   	    根据数据模板生成模拟数据。
   */
			Mock.mock = function (rurl, rtype, template, title) {
				// Mock.mock(template)
    ...
    */
				...
				string: function string(options) {
					...
					if (options.template.length) {
						...
						for (i = 0; i < placeholders.length; i++) {
							...
							// 只有一个占位符，并且没有其他字符
							if (placeholders.length === 1 && ph === result && (typeof phed === 'undefined' ? 'undefined' : (0, _typeof3.default)(phed)) !== (typeof result === 'undefined' ? 'undefined' : (0, _typeof3.default)(result))) {
								// 
								result = phed;
								break;
      ...
      */
					...
					// 递归引用数据模板中的属性
					if (templateContext && (typeof templateContext === 'undefined' ? 'undefined' : (0, _typeof3.default)(templateContext)) === 'object' && key in templateContext && _placeholder !== templateContext[key] // fix #15 避免自己依赖自己
					) {
							// 先计算被引用的属性值
				...
				getValueByKeyPath: function getValueByKeyPath(key, options) {
					...
					// 尚未计算，递归引用数据模板中的属性
					if (templateCurrentContext && (typeof templateCurrentContext === 'undefined' ? 'undefined' : (0, _typeof3.default)(templateCurrentContext)) === 'object' && key in templateCurrentContext && originalKey !== templateCurrentContext[key] // fix #15 避免自己依赖自己
					) {
							// 先计算被引用的属性值
     ...
     */
			...
			Util.extend(MockXMLHttpRequest.prototype, {
				...
				open: function open(method, url, async, username, password) {
					...
					var __title = item ? item.title ? item.title : '忘记起名字了?' : '我也不知道请求的啥';
					console.group && console.group("拦截到的请求: " + __title);
					console.log('| %c请求地址: %c' + this.custom.options.url, __color_1, __color_3);
					console.log('| %c请求方法 - 状态: %c%s - %c%s', __color_1, __color_3, this.custom.options.type, item ? __color_3 : __color_2, item ? 'HIT' : 'MISS');

					if (item) {
						console.log('| %c匹配url模版: %c' + item.rurl, __color_1, __color_3);
						console.log('| %c匹配数据模版: ', __color_1, item.template);
					}
				...
				send: function send(data) {
					...
					function done() {
						...
						that.response = that.responseText = (0, _stringify2.default)(convert(that.custom.template, that.custom.options), null, 4);
